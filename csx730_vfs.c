...
void cleanup(void)
{
    ...
    file_t * file;
    file_t * next;
    for (file = __global.files_head, next = file->next; file != __global.files_tail && file != NULL; file = next, next = file->next) {
        free(file);
    }
...
ssize_t csx730_read(fd_t fd, void *buf, size_t len)
{
    ...
    size_t disk_offset = sizeof(superblock_t) + sizeof(inode_t) * __global.inode_count + \
        file->inode->bno * DISK_BLOCK_SIZE + file->inode->offset + file->offset;
